...
| Guards | `netlify/lib/future-headline/guard.ts` | Signed page token, signed session cookie (25 per browser, presenter bypass via `FH_DEMO_CODE`), daily cap counter in Netlify Blobs, origin check. Netlify per-IP rate limit is set in the function's `config`. |
...
| `FH_DEMO_CODE` | no | 8+ characters. Opening the page as `/ai-summit/future-headline?demo=<code>` skips the per-browser cap for that tab (presenter use). The daily cap still applies. Rotate it if the link leaks. |
| `FH_SESSION_CAP` | no | Default 25 generations per browser per 12 hours (the handoff said 3; Dave raised it 2026-09-17). Server-enforced by signed cookie; the page reads the remaining count from the API. |
